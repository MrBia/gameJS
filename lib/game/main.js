...
MyGame = ig.Game.extend({
	...
	init: function() {
		...
		ig.input.bind( ig.KEY.DOWN_ARROW, 'pause' );
...
MyTitle = ig.Game.extend({
	...
	isHongKong: function(){
		...
		var cy = ig.system.height/2;
		return(
			(cx - this.hongkong.width/2 < ig.input.mouse.x && ig.input.mouse.x < cx - this.hongkong.width/2 + 100)&&
			(cy < ig.input.mouse.y && ig.input.mouse.y < cy+50)
		);
	},
	...
	isTaiWan:function(){
		...
		var cy = ig.system.height/2;
		return(
			(cx + this.hongkong.width + 50 < ig.input.mouse.x && ig.input.mouse.x < cx + this.hongkong.width + 50 + 100)&&
			(cy < ig.input.mouse.y && ig.input.mouse.y < cy+50)
		);
	},
	...
	draw: function(){
		...
		this.title.draw(cx - this.title.width/2, cy+80);
		this.china.draw(cx-250, cy);
		this.hongkong.draw(cx - this.hongkong.width/2, cy);
		this.taiwan.draw(cx + this.hongkong.width + 50, cy);
	}
});
...
GameComplete = GameEnd.extend({
	...
	init: function(){
		this.parent();
		this.right = ig.game.spawnEntity(EntityButtonRight, ig.system.width/2 + 50,
										   ig.system.height/2 +  this.BackgroundImage.height/2 - 60);
		
		this.left = ig.game.spawnEntity(EntityButtonLeft, ig.system.width/2 - 100,
										   ig.system.height/2 +  this.BackgroundImage.height/2 - 60);
	},
	...
	nextLevel: function(){
		if (ig.global[n] == LevelChina) {
            ig.global.numberBall = 50;
			ig.global.n = 'LevelHongkong';
        }
		else if (ig.global[n] == LevelHongkong) {
            ig.global.numberBall = 70;
			ig.global.n = 'LevelTaiwan';
        }
		else{
...
GamePause = GameEnd.extend({
	init: function(){
		this.parent();
		this.play = ig.game.spawnEntity(EntityButtonPlay, ig.system.width/2 - 20,
										   ig.system.height/2 +  this.BackgroundImage.height/32 );
	},
	...
	drawButtonPlay: function(){
		this.play.draw();
	},
	
	drawTitle: function(){
		var font =  new ig.Font( 'media/04b03.font.png' );
		font.draw('PAUSE', ig.system.width/2 - 20,
				  ig.system.height/2 - this.BackgroundImage.height/2 + 20);
	}
});
